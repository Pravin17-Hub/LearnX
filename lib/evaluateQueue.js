...
function getSystemPrompt(maxMarks) {
  return `You are an expert academic evaluator strictly following the official faculty grading rubric and model answer for each question:
...
3. SCORE BOUNDS:
   - Maximum Marks for this question: ${maxMarks}.0.
   - If unanswered, blank, or completely unrelated to the topic, award 0.0.
    ...
    for (const q of theoryQuestions) {
      ...
      if (studentAns.trim() !== '') {
        ...
        if (!refAnswer || refAnswer.trim() === '' || refAnswer === '[]') {
          refAnswer = 'A clear, logical, and accurate academic explanation demonstrating core concepts and working code examples.';
        }

        const systemPrompt = getSystemPrompt(q.points);
        const userContent = `Question: ${q.question_text}\nFaculty Answer Key & Rubric: ${refAnswer}\nMaximum Marks: ${q.points}\nStudent Answer: ${studentAns}`;
        ...
        try {
          ...
          itemFeedback.feedback = aiResult.feedback || 'AI evaluated successfully against faculty rubric.';
          itemFeedback.strengths = aiResult.strengths || '';
          itemFeedback.weaknesses = aiResult.weaknesses || '';
          ...
          // If queue worker encounters temporary error, record 0 and log for retry instead of arbitrary half-marks
          itemFeedback.score = 0;
          itemFeedback.feedback = 'Evaluation queued. Retrying evaluation shortly.';
        }
      } else {
